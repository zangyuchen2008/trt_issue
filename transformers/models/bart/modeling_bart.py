...
# for index in tqdm(range(12)):
#     trt_later_token_models[str(index)](decoder_layer_hidden_states,de_encoder_hidden_states
...
def _expand_mask(
    ...
    # ashape= extended_mask.shape
    # extended_mask =  extended_mask.view([ashape[0],ashape[1],ashape[3],ashape[2]]).type(torch.float32)
    # ones = torch.ones(ashape[:3]+ (tgt_len,),dtype=torch.float32).to(extended_mask.device)
...
class BartAttention(nn.Module):
    ...
    def _shape(self, tensor: torch.Tensor, seq_len: int, bsz: int):
        ...
        return torch.transpose(tensor.view(bsz, seq_len, self.num_heads, self.head_dim),1,2).contiguous()
    ...
    def forward(
       ...
       # get key, value proj
        ...
        attn_weights = torch.matmul(query_states, torch.transpose(key_states,2,3))
        # torch2trt modified
        ...
        # )
        attn_output = (
            # torch.transpose(attn_output.view(bsz, self.num_heads, tgt_len, self.head_dim),1,2)
            # .reshape(bsz, tgt_len, embed_dim)
            torch.transpose(attn_output,1,2).reshape(bsz, tgt_len, embed_dim)
        )
...
class BartEncoderLayer(nn.Module):
    def __init__(self, config: BartConfig):
        ...
        self.self_attn_layer_norm = torch.nn.LayerNorm(self.embed_dim)#BartLayerNorm(self.embed_dim)
...
# torch2trt modified
def simple_laynorm(method,x):
    weight = method.weight
    bias = method.bias
    mean = x.mean(dim=len(x.shape)-1,keepdim=True)
    var = ((x - mean)**2).mean(2,keepdim=True)
    return torch.mul(((x - mean)/torch.sqrt(var + 1e-05)),weight) + bias
...
class BartDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if not self.normalize_before:
            # torch2trt modified 1
            # hidden_states = self.self_attn_layer_norm(hidden_states)
            hidden_states = simple_laynorm(self.self_attn_layer_norm,hidden_states)
        ...
        if encoder_hidden_states is not None:
            ...
            if self.normalize_before:
                ...
                hidden_states = simple_laynorm(self.encoder_attn_layer_norm,hidden_states)
...
class BartDecoder(BartPretrainedModel):
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            # torch2trt modified
            s2 = time()
            if trt_models and hidden_states.shape[1] > 1: # note cuurent model is not using cache
                s0 = time()
                trt_model = trt_later_token_models[str(idx)]
                hidden_states = trt_model(hidden_states,encoder_hidden_states,
                encoder_attention_mask,combined_attention_mask)
                e0 = time()
                BartDecoder.trt_times.append({'token_index':hidden_states.shape[1],'layer_index':idx,
                'trt_time': e0-s0})
            else:
                s1 = time()
                ...
                if hidden_states.shape[1] > 1:
                    BartDecoder.raw_times.append({'token_index':hidden_states.shape[1],'layer_index':idx,
                    'raw_time':e1-s1})
            e2 = time()
            if trt_models:
                BartDecoder.full_trt_times.append({'token_index':hidden_states.shape[1],'layer_index':idx,
                'trt_time':e2-s2})
            else:
                BartDecoder.full_raw_times.append({'token_index':hidden_states.shape[1],'layer_index':idx,
                'raw_time':e2-s2})
...
class BartModel(BartPretrainedModel):
    ...
    def forward(
        ...
        e=time()
        if trt_models:
            BartModel.decoder_trt_times.append({'token_index':decoder_input_ids.shape[1],'raw_time':e-s}) 
        else:
            BartModel.decoder_raw_times.append({'token_index':decoder_input_ids.shape[1],'raw_time':e-s})        
...
class BartForConditionalGeneration(BartPretrainedModel):
    ...
    def forward(
        ...
        e= time()
        if trt_models:
            BartForConditionalGeneration.bart_trt_times.append({'trt_time':e-s}) 
        else:
            BartForConditionalGeneration.bart_raw_times.append({'raw_time':e-s})  
