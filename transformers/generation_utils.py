    ...
    def generate(
        ...
        **model_kwargs
        ...
        if is_greedy_gen_mode:
            ...
            e= time()
            # if trt_models:
            #     GenerationMixin.gen_trt_times.append({'trt_time':e-s}) 
            # else:
            #     GenerationMixin.gen_raw_times.append({'raw_time':e-s})  
            return result
    ...
    def beam_search(
        ...
        **model_kwargs
        ...
        while cur_len < max_length:
            model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
            # torch2trt modified
            model_inputs['trt_models']=trt_models
            # s= time()
            outputs = self(**model_inputs, return_dict=True)
            # e = time()
            # if trt_models:
            #     GenerationMixin.model_trt_times.append({'trt_time':e-s}) 
            # else:
            #     GenerationMixin.model_raw_times.append({'raw_time':e-s})  
            next_token_logits = outputs.logits[:, -1, :]
            ...
            # e = time()
            if trt_models:
                GenerationMixin.gen_trt_times.append([e1-s1,e2-s2,e3-s3,e4-s4,e5-s5,e6-s6]) 
            else:
                GenerationMixin.gen_raw_times.append([e1-s1,e2-s2,e3-s3,e4-s4,e5-s5,e6-s6])        

            if beam_scorer.is_done:
                break
        e0 = time()
        # if trt_models:
        #     GenerationMixin.gen_trt_times.append({'trt_time':e0-s0}) 
        # else:
        #     GenerationMixin.gen_raw_times.append({'raw_time':e0-s0}) 
