...
model_path = '/data/yuchen/projects/gns/model/test/model.onnx'
...
data = '''hello , my name is zang, it's a nice day and I'd like to meet you in the garden
'''
inputs = tokenizer(data, max_length=64, return_tensors='pt', padding='max_length', truncation=True)#'max_length'
...
# # BartEncoderlayer
# input_id_tensor = torch.tensor(np.random.normal(size =(1,128,1024))).type(torch.float32).to(device)
# attention_mask =  torch.ones((1,1,128,128),dtype=torch.int64).to(device)
...
def build_engine(model_file,trt_logger,max_ws=512*1024*1024, fp16=False):
    ...
    with trt.OnnxParser(network, TRT_LOGGER) as parser:
        with open(model_file, 'rb') as model:
            ...
            return engine
engine = build_engine(model_sim_path,TRT_LOGGER) #model_sim_path
