...
def allocate_buffers_modified(engine,dyinp_idx,dyinp_batch):
    inputs = []
    outputs = []
    bindings = []
    stream = cuda.Stream()
    for index,binding in enumerate(engine):
        if index == dyinp_idx and engine.binding_is_input(binding):
            size = trt.volume(engine.get_binding_shape(binding)) * engine.max_batch_size * dyinp_batch
        # elif not engine.binding_is_input(binding):
        #     size = trt.volume(engine.get_binding_shape(binding)) * engine.max_batch_size * dyinp_batch
        else:
