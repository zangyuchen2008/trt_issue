...
def check_view_first_dim_sqeeze(ctx):
    c1 = (ctx.method_str == 'torch.Tensor.view')
    input_shape = ctx.method_args[0].shape
    view_args = ctx.method_args[1:]
    c2 = (view_args[0] == input_shape[0] * input_shape[1])
    c3 = (view_args[1] == -1)
    c4 = (view_args[3] == input_shape[-1])
