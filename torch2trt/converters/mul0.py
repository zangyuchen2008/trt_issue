...
def convert_bool(ctx):
    ...
    input = ctx.method_args[1]
    output = ctx.method_return
    matrix = module.detach().cpu().numpy()
    inp = input.detach().cpu().numpy()
    embedding = np.asarray([matrix[index,:]  for index in inp[0]])
    layer = ctx.network.add_scale_nd(layer.get_output(0), trt.ScaleMode.CHANNEL, bias, weight,
    np.ones_like(weight), 0)
    output._trt = ctx.network.add_constant(embedding.shape, embedding).get_output(0)
