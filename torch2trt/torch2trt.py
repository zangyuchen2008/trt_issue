...
def add_missing_trt_tensors(network, tensors):
    ...
    for i, t in enumerate(tensors):
        ...
        if isinstance(t, float) or isinstance(t, int):
            ...
            # torch2trt modified
            if hasattr(scalar,'dtype') and scalar.dtype == 'int64': scalar = scalar.astype(np.int32)
        ...
        # or... add constant for leaf tensor w/o _trt
            ...
            # torch2trt modified
            if hasattr(weight,'dtype') and weight.dtype == 'int64': weight = weight.astype(np.int32)
...
class ConversionHook(object):
    ...
    def _set_method(self, method):
        ...
        # 这句话为torch中对应的方法(如relu)赋予了一个新的wrapper，这个wrapper
        # 来自method，method来自attach_converter函数，该函数确保了原torch函数的执行，并
        # 对torch原函数进行了tensorrt转换，
...
def torch2trt(module, 
    ...
    if use_onnx:
        ...
        with ConversionContext(network) as ctx:
            print('after ConversionContext lock status: {}'.format(ctx.lock))
            ctx.add_inputs(inputs, input_names)
            print('after ctx.add_inputs, lock status: {}'.format(ctx.lock))
            outputs = module(*inputs)
            print('after outputs = module(*inputs), lock status: {}'.format(ctx.lock))
            if not isinstance(outputs, tuple) and not isinstance(outputs, list):
                outputs = (outputs,)
